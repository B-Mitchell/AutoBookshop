...
const BookDetailsPage = ({ params }) => {
  ...
  const [isEbookModalOpen, setIsEbookModalOpen] = useState(false); // State for ebook modal
  ...
  // Handle order placement for physical book
  ...
  // Handle ebook order placement
  const handleEbookOrder = async (e) => {
    e.preventDefault();
    if (!user) {
      alert('Please log in to place an order');
      return;
    ...
    const orderData = {
      ...
      orderType: "ebook",
      status: 0,
      cost: book.ebookPrice,
    };
    const { error } = await supabase.from('orders').insert([orderData]);

    if (error) {
      console.error('Error placing ebook order:', error);
    } else {
      alert(`eBook order placed for: ${book.title}`);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 md:border mt-7 border-gray-300 rounded-md shadow-lg">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-2">
        ...
        <div className="p-4 border-r border-gray-300">
          ...
          <p className="text-xl font-semibold mb-4 text-green-700">NGN{book.amount}</p>
          ...
          <button 
            ...
            className="bg-green-600 text-white py-2 px-4 rounded-lg transition-all hover:bg-green-700 mr-3"
          >
            Place Order
          </button>
          <button 
            onClick={() => setIsEbookModalOpen(true)}
            className="bg-blue-600 text-white py-2 px-4 rounded-lg transition-all hover:bg-blue-700"
          >
            Place eBook Order
          </button>
        </div>
      ...
      {isEbookModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4">eBook Order Details</h2>
            <form onSubmit={handleEbookOrder}>
              <p className="text-md mb-2">Price: NGN{book.ebookPrice}</p>
              <div className="flex justify-between mt-6">
                <button 
                  type="button"
